...
FOUR.Viewcube = (function () {
    ...
    Viewcube.prototype.render = function () {
        ...
        // if(self.scene.getObjectByName('labels')!=undefined)
        //     self.scene.getObjectByName('labels').rotation.z++;
    };

    Viewcube.prototype.setupCamera = function () {
        var self = this;
        // position and point the camera to the center of the scene
        self.camera = new THREE.PerspectiveCamera(self.fov, self.domElement.clientWidth / self.domElement.clientHeight, 0.1, 3000);
    ...
    Viewcube.prototype.setupGeometry = function () {
        ...
        self.components=new THREE.Object3D();
        ...
        self.cube.name='cube';
        // add
        self.components.add(self.cube);
        self.components.add(self.compass);

        self.scene.add(self.components);
    };
    ...
    Viewcube.prototype.setView = function (view) {
        ...
        switch (view) {
            case 10://top
                self.camera.position.set(0, 0, self.OFFSET);
                self.camera.lookAt(0,0,0);
                // self.tweenCameraToPosition(0,0,self.OFFSET);
                break;
            case 2://right
                self.camera.position.set(0, self.OFFSET, 0);
                self.camera.lookAt(0,0,0);

                // self.tweenCameraToPosition(self.OFFSET,0,0);
                break;
            case 6://front
                self.camera.position.set(self.OFFSET, 0, 0);
                self.camera.lookAt(0,0,0);
