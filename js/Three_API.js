...
(function () {
    ...
    var editViewWidth,
        editViewHeight,
        camera, camControls, scene, renderer, modelScale;
    var clock;
    ...
    let raycasterIntersects = [];  //该数组用于存放通过Raycaster对场景物体求交的结果
    ...
    function initThreeJSScene(CanvasDom,dbclickEvent,webGLWidthScale=1) {
        if(!scene){
            ...
            CanvasDom.ondblclick = function(event){
                ...
                if(true)//可以判断编辑模式
                {
                    ...
                    for(var editInfoSelectedObjCount = 0;editInfoSelectedObjCount< editInfoSelectedObj.length;editInfoSelectedObjCount++)
                    {
                        if(editInfoSelectedObj[editInfoSelectedObjCount] == tempNewComponent)
                        {
                            //当前点击的物体是已经高亮显示的物体
    ...
    function initModel(modelName,modelScaleValue) {
        ...
        Three_Api.productFactor = 1/modelScale;
    }
    ...
    workerLoadVsg.onmessage = function (event) {
        ...
        for(var k in event.data.vsgMap) {
            ...
            for(var i =0;i<vsgList.length;i++){
                if(Three_Api.vsgArr.indexOf(vsgList[i]) == -1) Three_Api.vsgArr.push(vsgList[i]);
            }
        }
        Three_Api.sceneConfig.vsg = event.data.vsgMap;
        Three_Api.sceneConfig.voxelSize = event.data.voxelSize;
        Three_Api.sceneConfig.sceneBBoxMinX = event.data.sceneBBoxMinX;
        Three_Api.sceneConfig.sceneBBoxMinY = event.data.sceneBBoxMinY;
        Three_Api.sceneConfig.sceneBBoxMinZ = event.data.sceneBBoxMinZ;
    ...
    workerLoadMergedFile.onmessage = function (event) {
        ...
        if(Data.data_tag!=null)
        {
            if(Data.data_tag==1) {
                ...
                if(packageTag>=23){
                    for(var drawThreadCount = 0; drawThreadCount<23; drawThreadCount++)
                    {
        ...
        else
        {
            ...
            if(Data.newFileName)
            {
                ...
                if(!Three_Api.modelDataNewN[currentBlockName]) Three_Api.modelDataNewN[currentBlockName] = {};
                if(!Three_Api.modelDataM[currentBlockName]) Three_Api.modelDataM[currentBlockName] = {};
                if(!Three_Api.modelDataNewN[currentBlockName][tempKeyValue])
                {
                    Three_Api.modelDataNewN[currentBlockName][tempKeyValue] = [];
                }
                if(!Three_Api.modelDataM[currentBlockName][tempKeyValue])
                {
                    Three_Api.modelDataM[currentBlockName][tempKeyValue] = [];
                }
                Three_Api.modelDataNewN[currentBlockName][tempKeyValue] = Data.newFileName;
                Three_Api.modelDataM[currentBlockName][tempKeyValue] = Data.m;
            }
            else{
                var tempKeyValue = Data.nam;
                if(!Three_Api.modelDataV[currentBlockName]) Three_Api.modelDataV[currentBlockName] = {};
                if(!Three_Api.modelDataT[currentBlockName]) Three_Api.modelDataT[currentBlockName] = {};
                if(!Three_Api.modelDataF[currentBlockName]) Three_Api.modelDataF[currentBlockName] = {};
                if(!Three_Api.modelDataV[currentBlockName][tempKeyValue])
                {
                    Three_Api.modelDataV[currentBlockName][tempKeyValue] = [];
                }
                if(!Three_Api.modelDataT[currentBlockName][tempKeyValue])
                {
                    Three_Api.modelDataT[currentBlockName][tempKeyValue] = [];
                }
                if(!Three_Api.modelDataF[currentBlockName][tempKeyValue])
                {
                    Three_Api.modelDataF[currentBlockName][tempKeyValue] = [];
                }
                for(var dataCount = 0; dataCount<Data.v.length;dataCount++)
                {
                    Three_Api.modelDataV[currentBlockName][tempKeyValue].push(Data.v[dataCount]);
                    Three_Api.modelDataT[currentBlockName][tempKeyValue].push(Data.t[dataCount]);
                    Three_Api.modelDataF[currentBlockName][tempKeyValue].push(Data.f[dataCount]);
                }
            }
            Data = null;
        }
    ...
    workerDrawBIM.onmessage = function (event) {
        ...
        if(drawCount==23)
        {
            ...
            productFactor = Three_Api.productFactor ? Three_Api.productFactor : 1/0.0006 ;
            ...
            //根据用户以及打开的场景来加载摄像机的位置
            // var descriptionUrl = hostIP+"/returnCameraLocation?buildingName="+currentBlockName;
            // $.get(descriptionUrl,(resText)=> {
            //     //删除table的所有行
            //     var tb = document.getElementById('camPositionTable');
            //     // tb.onclick = ()=>console.log('click table');
            ...
            //     camPositionNameArr = [];
            //     if(resText.length>0){
            //         console.log("读取摄像机位置:" + resText[0]);
            //         camPositionArr = JSON.parse(resText[0].pos_arr);
            //         camPositionNameArr = JSON.parse(resText[0].name_arr);
            //
            //         for (var posCount = 0; posCount < camPositionNameArr.length; posCount++)
            //         {
            //             var tr=document.getElementById("camPositionTable").insertRow();
            //             var td1=tr.insertCell();
            //             var td2=tr.insertCell();
            //
            //             //不需要用全局变量进行记录，直接用camposition的数组计算就行
            //             var saveCamNum = posCount;
            //
            //             td1.innerHTML='<button class="jump-button" id="bt">'+camPositionNameArr[posCount]+' </button>';
            //
            //             // document.getElementById("bt").id=saveCamNum.toString();
            //             document.getElementById("bt").id='c'+saveCamNum.toString();
            //
            //             document.getElementById('c'+saveCamNum.toString()).onclick =function jumpToPosition(event) {
            //                 var indexStr = event.currentTarget.id.substring(1);
            //                 var tempIndex = Number(indexStr)*3;
            //                 camera.position.set(camPositionArr[tempIndex],camPositionArr[tempIndex+1],camPositionArr[tempIndex+2]);
            //             };
            //             td2.innerText='('+camPositionArr[saveCamNum*3]+','+camPositionArr[saveCamNum*3+1]+','+camPositionArr[saveCamNum*3+2]+')';
            //         }
            //     }
            // });
        ...
        if(drawDataMap[currentBlockName][tag].length>0)
        {
            ...
            if(tempName)
            {
                ...
                for(var i=0; i<drawDataMap[currentBlockName][tag].length; i++)
                {
                    ...
                    {
                        if (Three_Api.modelDataNewN[currentBlockName] && Three_Api.modelDataNewN[currentBlockName][tempFileName]) {

                            var newName = Three_Api.modelDataNewN[currentBlockName][tempFileName];
                            var matrix = Three_Api.modelDataM[currentBlockName][tempFileName];
//                            处理V矩阵，变形
                            if(Three_Api.modelDataV[currentBlockName][newName])
                            {
                                Three_Api.modelDataV[currentBlockName][tempFileName] = [];
                                for(var dataCount=0;dataCount<Three_Api.modelDataV[currentBlockName][newName].length;dataCount++)
                                {
                                    var vArrary = [];
                                    var fArrary = [];
                                    var vMetrix = [];
                                    var fMetrix = [];
                                    for (var j = 0; j < Three_Api.modelDataV[currentBlockName][newName][dataCount].length; j += 3) {
                                        var newN1 = Three_Api.modelDataV[currentBlockName][newName][dataCount][j] * matrix[0] + Three_Api.modelDataV[currentBlockName][newName][dataCount][j + 1] * matrix[4] + Three_Api.modelDataV[currentBlockName][newName][dataCount][j + 2] * matrix[8] + 1.0 * matrix[12];
                                        var newN2 = Three_Api.modelDataV[currentBlockName][newName][dataCount][j] * matrix[1] + Three_Api.modelDataV[currentBlockName][newName][dataCount][j + 1] * matrix[5] + Three_Api.modelDataV[currentBlockName][newName][dataCount][j + 2] * matrix[9] + 1.0 * matrix[13];
                                        var newN3 = Three_Api.modelDataV[currentBlockName][newName][dataCount][j] * matrix[2] + Three_Api.modelDataV[currentBlockName][newName][dataCount][j + 1] * matrix[6] + Three_Api.modelDataV[currentBlockName][newName][dataCount][j + 2] * matrix[10]+ 1.0 * matrix[14];
                                        vArrary.push(newN1);
                                        vArrary.push(newN2);
                                        vArrary.push(newN3);
                                        var groupV;
                                    ...
                                    //处理T矩阵
                                    for (var m = 0; m < Three_Api.modelDataT[currentBlockName][newName][dataCount].length; m += 3) {
                                        var newT1 = 1.0 * Three_Api.modelDataT[currentBlockName][newName][dataCount][m];
                                        var newT2 = 1.0 * Three_Api.modelDataT[currentBlockName][newName][dataCount][m + 1];
                                        var newT3 = 1.0 * Three_Api.modelDataT[currentBlockName][newName][dataCount][m + 2];
                        ...
                        else if (Three_Api.modelDataV[currentBlockName][tempFileName] && !(Three_Api.modelDataNewN[currentBlockName] && Three_Api.modelDataNewN[currentBlockName][tempFileName])) {

                            for(var dataCount=0;dataCount<Three_Api.modelDataV[currentBlockName][tempFileName].length;dataCount++)
                            {
                                var vArrary = [];
                                var fArrary = [];
                                var vMetrix = [];
                                var fMetrix = [];
                                //处理V矩阵，变形
                                for (var j = 0; j < Three_Api.modelDataV[currentBlockName][tempFileName][dataCount].length; j += 3) {
                                    var newn1 = 1.0 * Three_Api.modelDataV[currentBlockName][tempFileName][dataCount][j];
                                    var newn2 = 1.0 * Three_Api.modelDataV[currentBlockName][tempFileName][dataCount][j + 1];
                                    var newn3 = 1.0 * Three_Api.modelDataV[currentBlockName][tempFileName][dataCount][j + 2];
                                    var groupV;
                                    // if(did==4)  groupV = new THREE.Vector3(newn1+249299, newn3-17125, newn2+223363);
                                    // else
                                    groupV = new THREE.Vector3(newn1, newn3, newn2);
                                ...
                                //处理T矩阵
                                for (var m = 0; m < Three_Api.modelDataT[currentBlockName][tempFileName][dataCount].length; m += 3) {
                                    var newT1 = 1.0 * Three_Api.modelDataT[currentBlockName][tempFileName][dataCount][m];
                                    var newT2 = 1.0 * Three_Api.modelDataT[currentBlockName][tempFileName][dataCount][m + 1];
                                    var newT3 = 1.0 * Three_Api.modelDataT[currentBlockName][tempFileName][dataCount][m + 2];
                                    var grouT = new THREE.Face3(newT1, newT2, newT3);
                                    fArrary.push(newT1);
                                    fArrary.push(newT2);
                                    fArrary.push(newT3);
    ...
    function destoryModel() {
        ...
        Three_Api.productFactor = 0 ; //用于修正vsg与场景比例的差异
        vsgArr = [];
        Three_Api.vsgArr = [];
        ...
        Three_Api.sceneConfig = new sceneConfigInfo();
        modelDataV = [];
        modelDataT = [];
        modelDataF = [];
        modelDataM = [];
        modelDataNewN = [];
        Three_Api.modelDataV = [];
        Three_Api.modelDataT = [];
        Three_Api.modelDataF = [];
        Three_Api.modelDataM = [];
        Three_Api.modelDataNewN = [];
        currentBlockName = "";
        drawDataMap = {};
        packageTag = 0;
        drawCount = 0;
    ...
    function SaveCamPosition(cnName,SaveCallback) {
        ...
        //
        // //不需要用全局变量进行记录，直接用camposition的数组计算就行
        // var saveCamNum = (camPositionArr.length / 3)-1;
        //
        // td1.innerHTML='<button class="jump-button" id="bt">'+string+' </button>';
        //
        // // document.getElementById("bt").id=saveCamNum.toString();
        // document.getElementById("bt").id='c'+saveCamNum.toString();
        //
        ...
        //     var tempIndex = Number(indexStr)*3;
        //     camera.position.set(camPositionArr[tempIndex],camPositionArr[tempIndex+1],camPositionArr[tempIndex+2]);
        //     // camControls.targetObject.position.set(camPosition[tempIndex],camPosition[tempIndex+1],camPosition[tempIndex+2]);
        //     // camControls.lon = camPosition[tempIndex+3];
        //     // camControls.lat = camPosition[tempIndex+4];
        // };
        // td2.innerText='('+camPositionArr[saveCamNum*3]+','+camPositionArr[saveCamNum*3+1]+','+camPositionArr[saveCamNum*3+2]+')';
        //
        // $('#inputNameDiv').modal('hide');
        // // document.getElementById('inputNameDiv').style.display = "none";
        // document.getElementById('camSaveBtnName').value.value="";
    }


//剖切面事件处理程序
    //击中剖面，则附上Gizmos，否则删除Gizmos
    //？？尚不确定是否必须关联剖切开关
    //剖切辅助测量的面板控制
    //UI部分
    //planebutton表示开关
    // document.getElementById("planebutton").onclick = function () {
    //     //打开测量平面
    //     if (document.getElementById("planebutton").innerHTML === "测量") {
    //         document.getElementById("plane_position").style.display = "block";
    //         document.getElementById("planebutton").innerHTML = "关闭";
    //         document.querySelector("#deleteTagControl").onclick = auxiliaryMeasuringEvent.deleteTagControlOnClick;
    //         document.querySelector("#measureControl").onclick = auxiliaryMeasuringEvent.measureControlOnClick;
    //         document.querySelector("#measureHeight").onclick = auxiliaryMeasuringEvent.measureHeightOnClick;
    //
    //     }
    //     //关闭测量平面
    //     else {
    //         document.getElementById("planebutton").innerHTML = "测量";
    //         document.getElementById("plane_position").style.display = "none";
    //         document.querySelector("#deleteTagControl").onclick = null;
    //         document.querySelector("#measureControl").onclick = null;
    //         document.querySelector("#measureHeight").onclick = null;
...
//                console.log(this._visiblePlane.matrix.elements[14]);
    ...
    //剖切UI
    //剖切按钮将对测量按钮产生影响，若关闭剖切功能时测量面板仍然开启，需要卸载测量平面的相关事件
    // document.getElementById("clippingToggle").onclick = () => {
    //     const dom = document.querySelector("#planebutton");
    //     if (clippingPlanes.length > 0){
    //         dom.disabled = true;
    //         disableClipping();
    //         if (document.getElementById("planebutton").innerHTML !== "测量") {
    ...
    let enableSprite = false;   //是否开启精灵功能
    ...
    const dblClickWhenSpriteEnabled = function (event) {
        if (enableSprite) {
            ...
            if (selectPoint) {
                const spriteMap = new THREE.TextureLoader().load("/images/sprite0.png");
    ...
    //     {
    //         enableSprite = false;
    //         document.getElementById('sprites').innerText = "精灵球关闭";
    //         spritesArray.forEach(function(value){
    //             value.visible= false;
    //         });
    //         //添加精灵球
    //         document.getElementById('WebGL-Output').removeEventListener("dblclick" ,dblClickWhenSpriteEnabled );
    //         //检测精灵球点击
    //         document.getElementById('WebGL-Output').removeEventListener("click" , clickWhenSpriteEnabled);
    //     }
    //     else
    //     {
    //         enableSprite = true;
    //         document.getElementById('sprites').innerText = "精灵球";
    //         spritesArray.forEach(function(value){
    //             value.visible=true;
    //         });
    //         //添加精灵球
    //         document.getElementById('WebGL-Output').addEventListener("dblclick" ,dblClickWhenSpriteEnabled );
    //         //检测精灵球点击
    //         document.getElementById('WebGL-Output').addEventListener("click" , clickWhenSpriteEnabled);
    //     }
    // };
    ...
    var smcComponentDes = "";//全局变量，用于存放显示描述信息
    var smcBody = "";//全局变量，用于存放下载下来的语义信息

    function getComponentSMC(fileName) {
        ...
        $.get(url,(resText)=>{
            ...
            // {
            //     var tempStr = key + ":"  + result[key];
            //     showArr.push(tempStr);
            // }
            // $(".smc-body-text").html(showArr.join("<br\>"));
            // var img = document.getElementById("updateImage");
            // img.src = "assets/UI/pic_upload.png";
            var descriptionUrl = hostIP + "/returnDescription?collection="+currentBlockName+"_smc_describe&name=" + fileName;
            $.get(descriptionUrl,(resText)=> {
                // smcComponentName = fileName;
                // document.getElementById("descrptionContent").value = "";
                // if(resText.length>0 && resText[0].description)
                // {
                //     document.getElementById("descrptionContent").value = resText[0].description;
                //     smcComponentDes = resText[0].description;
                //     if(resText[0].picPath)
                //     {
                //         var img = document.getElementById("updateImage");
                //         img.src = hostIP+"/"+resText[0].picPath;
                //     }
                //     else
                //     {
                //         var img = document.getElementById("updateImage");
                ...
                //     }
                // }
                // $('#display_SMC').modal('show');
                // document.getElementById("componentName").value = currentBlockName+"_"+fileName;
                // console.log("当前查看的构件名称为: " + document.getElementById("componentName").value);
     ...
     */
    var searchResultArr = [];
    function SearchComponents(searchStr) {
        var url = hostIP+"/selectSet?collection="+currentBlockName+"_smc&name=" + searchStr;
        $.get(url,(resText)=> {
            var searchContentDiv = document.getElementById("search-body");
            if(resText.length>0)
            {
                searchContentDiv.innerHTML = "";
                searchResultArr = resText;
                searchContentDiv.value = "";
                for(var i=0; i<resText.length; i++)
                {
                    //添加 li
                    var component = document.createElement("input");
                    component.type="button";
                    component.value=resText[i].name;
                    component.id=resText[i].name;
                    component.className = "btn btn-primary";
                    component.onclick = function (event) {
                        $("#descript-body").empty();
                        var tempSingleComponent;
                        for(var resultCount = 0; resultCount<searchResultArr.length; resultCount++){
                            if(searchResultArr[resultCount].name == event.target.id){
                                tempSingleComponent = searchResultArr[resultCount].smc;
                            }
                        }
                        var showArr = [];
                        for(var key in tempSingleComponent)
                        {
                            var tempStr = key + ":"  + tempSingleComponent[key];
                            showArr.push(tempStr);
                        }
                        $("#descript-body").html(showArr.join("<br\>"));
                    };
                    searchContentDiv.appendChild(component);
                }
            }
            else {
                //没有结果
                searchContentDiv.innerHTML = "没有找到对应的结果";
            }
        });
    }


    //需要给查看构件语义信息的窗口加一个事件监听，在窗口消失之后，让场景中被选中的物体取消选中状态
    // $('#display_SMC').on('hidden.bs.modal', function () {
    //     // 让场景中被选中的物体取消选中状态
    //     scene.remove(editInfoSelectedObj[editInfoSelectedObj.length-1]);
    //     editInfoSelectedObj = [];
    // })
    //
    // document.getElementById("submitDes").onclick=function(){
    //     var form = document.getElementById("descriptForm");
    //     var formData = new FormData(form);
    //     var xhr = new XMLHttpRequest();
    //
    //     xhr.open("POST",hostIP+"/uploads");
    ...
    //             $('#display_SMC').modal('hide');
    //             var file = doucment.getElementById('fileInput');
    ...
    function parseNewFormatDat(str){
        ...
        // str.match(/{.*?}}/g).forEach((e)=>{  //这里是按构件细分
        //     tempComponent = JSON.parse(e);    //取出其中一个构件，将其转换为对象
        //     console.log(tempComponent.smc);
        // });
        return str[0].smc;
    }
    ...
    function getComponentByNameAndPoint(name, point) {
        ...
        var indexX = Math.ceil((point.x * productFactor - sceneConfig.sceneBBoxMinX )/sceneConfig.voxelSize) ;
        var indexZ = Math.ceil((point.z * productFactor - sceneConfig.sceneBBoxMinY )/sceneConfig.voxelSize) ;
        var indexY = Math.ceil((point.y * productFactor - sceneConfig.sceneBBoxMinZ )/sceneConfig.voxelSize) ;
        var index = indexX + "-" + indexZ + "-" + indexY;
        var voxelizationFileArr = sceneConfig.vsg[index];
        if(voxelizationFileArr)
        {
            for(var i=0; i<voxelizationFileArr.length; i++)
            {
                if(unDisplayModelArr.indexOf(voxelizationFileArr[i])==-1)
                {
                    ...
                    if(ind==componentType)
                    {
                        ...
                        var newObj = drawModelByFileName(currentBlock,voxelizationFileArr[i]);
                        scene.remove(newObj);
            ...
            tempArray[tempIndex].position.divideScalar(productFactor);
            tempArray[tempIndex].scale.divideScalar(productFactor);
     ...
     * @returns {*}
     */
    function drawModelByFileName(currentBlockName,fileName,dataInfo) {
        ...
        if(tempFileName!=null)
        {
            if (Three_Api.modelDataNewN[currentBlockName] && Three_Api.modelDataNewN[currentBlockName][tempFileName]) {
                ...
                var matrix = Three_Api.modelDataM[currentBlockName][tempFileName];
//                            处理V矩阵，变形
                if(Three_Api.modelDataV[newName])
                {
                    var centerPos;
                    var vMetrixArr = [];
                    var allVMetrix = [];
                    ...
                    for(var dataCount=0;dataCount<Three_Api.modelDataV[newName].length;dataCount++)
                    {
                        var vMetrix = [];
                        var tMetrix = [];
                        var uvArray = [];
                        ...
                        //处理T矩阵
                        for (var m = 0; m < Three_Api.modelDataT[currentBlockName][newName][dataCount].length; m += 3) {
                            var newT1 = 1.0 * Three_Api.modelDataT[currentBlockName][currentBlockName][newName][dataCount][m];
                            var newT2 = 1.0 * Three_Api.modelDataT[currentBlockName][newName][dataCount][m + 1];
                            var newT3 = 1.0 * Three_Api.modelDataT[currentBlockName][newName][dataCount][m + 2];
                            var newF1 = 1.0 * Three_Api.modelDataF[currentBlockName][newName][dataCount][m];
                            var newF2 = 1.0 * Three_Api.modelDataF[currentBlockName][newName][dataCount][m + 1];
                            var newF3 = 1.0 * Three_Api.modelDataF[currentBlockName][newName][dataCount][m + 2];
                ...
                var modelGeo = new THREE.Geometry();
                for(var dataCount=0;dataCount<Three_Api.modelDataV[currentBlockName][tempFileName].length;dataCount++) {
                    var singleMeshVMetrix = getVertexArrByVertexData(Three_Api.modelDataV[currentBlockName][tempFileName][dataCount]);
                    //得到V矩阵
                ...
                for(var dataCount=0;dataCount<Three_Api.modelDataV[currentBlockName][tempFileName].length;dataCount++)
                {
                    ...
                    for (var m = 0; m < Three_Api.modelDataT[currentBlockName][tempFileName][dataCount].length; m += 3) {
                        ...
                        var newT2 = 1.0 * Three_Api.modelDataT[currentBlockName][tempFileName][dataCount][m + 1];
                        var newT3 = 1.0 * Three_Api.modelDataT[currentBlockName][tempFileName][dataCount][m + 2];
                        var newF1 = 1.0 * Three_Api.modelDataF[currentBlockName][tempFileName][dataCount][m];
                        var newF2 = 1.0 * Three_Api.modelDataF[currentBlockName][tempFileName][dataCount][m + 1];
                        var newF3 = 1.0 * Three_Api.modelDataF[currentBlockName][tempFileName][dataCount][m + 2];
                        var norRow = new THREE.Vector3(newF1, newF3, newF2);
    ...
    function createMesh(geom,block,nam,isBuffer) {
        ...
        var material_transparent = new THREE.MeshPhongMaterial( { color: color,side: THREE.DoubleSide,shininess:5000,opacity:0.8,transparent:true});
        ...
        switch (nam) {
            ...
            case "IfcWallStandardCase"://ok
                ...
                tempBufferGeo.fromGeometry(geom);
                if(isBuffer)    mesh = new THREE.Mesh(tempBufferGeo, material3_2);
            ...
            case "IfcSlab"://ok
                ...
                tempBufferGeo.fromGeometry(geom);
                if(isBuffer)    mesh = new THREE.Mesh(tempBufferGeo, material0);
            ...
            case "IfcColumn"://ok
                ...
                tempBufferGeo.fromGeometry(geom);
                if(isBuffer)    mesh = new THREE.Mesh(tempBufferGeo, material4);
                else mesh = new THREE.Mesh(geom, material4);
                break;
            case "IfcPlate"://ok
                tempBufferGeo.fromGeometry(geom);
                if(isBuffer)    mesh = new THREE.Mesh(tempBufferGeo, material_transparent);
        ...
        document.getElementById("mapLoadID").onchange=function()
        {
            for(var i = 0;i<editInfoSelectedObj.length; i++) {
                switch (this.value) {
                    case "1":
                        SavedMaterial.push(editInfoSelectedObj[i].material);
                        editInfoSelectedObj[i].material.map = texture;
                        break;
                    case "2":
                        SavedMaterial.push(editInfoSelectedObj[i].material);
                        editInfoSelectedObj[i].material.map = texture1;
                        break;
                    case "3":
                        SavedMaterial.push(editInfoSelectedObj[i].material);
                        editInfoSelectedObj[i].material.map = texture2;
                        break;
                    case "4":
                        SavedMaterial.push(editInfoSelectedObj[i].material);
                        editInfoSelectedObj[i].material.map = texture3;
                        break;
                }
            }
            for(var i = 0;i<editInfoSelectedObj.length-1; i++) {
    ...
    function ChangeView() {
        ...
        {
            case texture:document.getElementById("mapLoadID").value = 1;document.getElementById("mapID").checked = true;break;
            case texture1:document.getElementById("mapLoadID").value = 2;document.getElementById("mapID").checked = true;break;
            case texture2:document.getElementById("mapLoadID").value = 3;document.getElementById("mapID").checked = true;break;
            case texture3:document.getElementById("mapLoadID").value = 4;document.getElementById("mapID").checked = true;break;
    ...
    //改变摄像机的俯仰视角，0是俯视，1是仰视
    // function ChangeCameraPos(type) {
    //     switch (type){
    //         case 0:
    ...
    // }
    function ChangeCameraPos(posX,posY,posZ) {
    ...
    function ShowLineModel() {
        ...
        for(var i=0;i<threeModelGroup.children.length;i++){
            var edges = new THREE.EdgesGeometry( threeModelGroup.children[i].geometry,10 );
            lineMesh.add(new THREE.LineSegments( edges, new THREE.LineBasicMaterial( { color: 0xffffff } ) ));
        }
