...
(function () {
...

    {
        ...
        var material3_2 = new THREE.MeshPhongMaterial( { color: 0xffffff, map: texture3_2,side: THREE.DoubleSide,shininess:100,opacity:1,transparent:false});
    ...
    workerDrawBIM.onmessage = function (event) {
        ...
        if(drawCount==24)
        {
            ...
            destroyAllSprite();
            finishDrawCallback();
            productFactor = productFactor ? productFactor : 1/0.0006 ;
...
//                            处理V矩阵，变形
    ...
    function destoryModel() {
        ...
        sceneConfig = new sceneConfigInfo();
        modelDataV = [];
        modelDataT = [];
        modelDataF = [];
        modelDataM = [];
...
//剖切面事件处理程序
     ...
     * @constructor
     */
    function GetSearchComponentsCount(searchStr,callback){
        var url = hostIP+"/selectSetCount?collection="+currentBlockName+"_smc&name=" + searchStr;
        $.get(url,(resText)=> {
            callback(resText);
        });
    }
    function SearchComponents(searchStr,page,callback) {
        var url = hostIP+"/selectSet?collection="+currentBlockName+"_smc&name=" + searchStr+"&page="+page;
        $.get(url,(resText)=> {
            if(resText.length>0) {
                callback(resText);
            } else {
...
//                            处理V矩阵，变形
    ...
    function createMesh(geom,block,nam,isBuffer) {
        ...
        var material_transparent = new THREE.MeshPhongMaterial( { color: color,side: THREE.FrontSide,shininess:5000,opacity:0.8,transparent:true,depthWrite:true});
    ...
    function redrawComponentsCollectionByFileNameList(filenamelist,matUrl,repetVal,opacityVal) {
        ...
        for(var count=0; count<polyhedrons.length; count++){
            if(polyhedrons[count].name.indexOf(typeName)!=-1){
    ...
    function getNewDataVByCnterPosAndVertexArr(centerPos,vertexArr) {
        ...
        for(var i=0;i<vertexArr.length; i++)
        {
            ...
            newDataV.push((vertexArr[i].x*modelScale - centerPos.x)*1.1);
            newDataV.push((vertexArr[i].y*modelScale - centerPos.y)*1.1);
            newDataV.push((vertexArr[i].z*modelScale - centerPos.z)*1.1);
        }
        return newDataV;
     ...
     * @returns 目标构件的坐标
     */
    function locatedToComponent(fileName,distance) {
        //因为不清楚要不要画模型出来，所以先不调用drawModelByFileName()了
        locatedModel = null;
        locatedModel = drawModelByFileName(currentBlockName,fileName);
        if(locatedModel){
    ...
    function getBuildingHeight() {
        var height = sceneConfig.sceneBBoxMaxZ/modelScale;
        console.log("当前建筑物高度为："+height);
        return
    }
